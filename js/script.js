...
// 4-5-6 задание
const movies_name = document.querySelectorAll(".promo__interactive-item")
...
function reload(arr, place) {
    ...
    for(let i = 0; i < arr.length; i++) {
        ...
        const div_for_divs = document.createElement("div")

        const imgPos = document.createElement("img")
        imgPos.classList.add("imgpos")
        imgPos.src = sorted[i].Poster

        const div = document.createElement("div")
        div.classList.add("div")
        div.innerHTML = sorted[i].Plot

        const actors = document.createElement("div")
        actors.classList.add("div")
        actors.innerHTML = sorted[i].Actors

        const writer = document.createElement("div")
        writer.classList.add("div")
        writer.innerHTML = sorted[i].Writer

        const production = document.createElement("div")
        production.classList.add("div")
        production.innerHTML = sorted[i].Production
        ...
        div_for_divs.append(div, actors, writer, production)
        mdiv.append(imgPos, div_for_divs, modalclose)
        modal.append(mdiv)
        body.append(modal)
